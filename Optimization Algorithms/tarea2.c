...
int main(){
    ...
    double dist(int n, double x[1][n]){
        ...
        // f\left(x\right)=\sum_{i=1}^{n-1}\left[100\left(x_{i+1}-x_{i}^{2}\right)^{2}+\left(1-x_{i}\right)^{2}\right],para\ n=7
        // https://www.desmos.com/calculator/qe4uujynhw
        double x_sum = 0;
        for(int i=1; i<n; i++){
            x_sum = x_sum + 100*pow(x[0][i+1]-pow(x[0][i],2),2) + pow(1-x[0][i],2);
        }
